...
import { Search, ShoppingBag, User, Star, TrendingUp, Sparkles, ChevronRight, Info } from 'lucide-react';
...
const ProductCard = ({ product, onClick }) => (
  ...
  >
    <div className="relative aspect-square bg-slate-800">
      <img 
        src={product.img_url || 'https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=500&q=80'} 
        alt={product.title}
...
const App = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [pRecs, tItems] = await Promise.all([
        axios.get(`${API_BASE}/recommend/user/${currentUser}?limit=5`),
        axios.get(`${API_BASE}/recommend/trending?limit=8`)
      ]);
      setPersonalizedRecs(pRecs.data.recommendations);
      setTrendingItems(tItems.data.recommendations);
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200">
      ...
      <nav className="sticky top-0 z-50 glass border-b border-white/5 px-6 py-4 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center bg-white/5 border border-white/10 rounded-full px-4 py-1.5 w-96 focus-within:border-indigo-500/50 transition-colors">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
            {loading ? (
              ...
              personalizedRecs.map((prod) => (
                <ProductCard key={prod.asin} product={prod} onClick={setSelectedProduct} />
              ))
            )}
          </div>
        </section>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              <div className="md:w-1/2 aspect-square md:aspect-auto bg-slate-900">
                <img 
                  src={selectedProduct.img_url || 'https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=800&q=80'} 
